...
def preprocess_input(frame, bbox, landmarks):
	...
	face_img = torch.from_numpy(face_img).float().unsqueeze(0).to(args.device)
	return face_img
...
def parse_args():
	...
	parser.add_argument('--data_dir', type=str, default='dataset/gaze360/', help='data directory')
...
while True:
	...
	if (bboxes.size and landmarks.size):	
		for bbox, landmark in zip(bboxes, landmarks):
			...
			with torch.no_grad():
				pred_gaze = model(face_img)			
			gaze = pred_gaze[0].cpu().numpy()
